...
# How about we add an AR(1) process for the mean?
spec11 <- ugarchspec(variance.model=list(model="sGARCH", garchOrder=c(1, 0)),
                     mean.model=list(armaOrder=c(1, 0, 0), 
                                     ...
                                     include.mean=TRUE), distribution.model = "norm")
# Let's fit our simple GARCH Model
fit11 <- ugarchfit(data=y,spec=spec11)
n.ahead <- length((length(to.use) + 1):n)
forc <- ugarchforecast(fit11, n.ahead = n.ahead)
